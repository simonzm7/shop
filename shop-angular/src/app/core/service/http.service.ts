...
export const CONTENT_TYPE_ENCODED = 'application/x-www-form-urlencoded';
...
export class HttpService {
  ...
  public createDefaultOptions(): Options {
    return {
      headers: new HttpHeaders({ 'Content-Type': CONTENT_TYPE_JSON })
    };
  }
  ...
  private createOptions(opts?: Options): Options {
    ...
    if (opts) {
      ...
      if (!opts.headers?.get('Content-Type')) {
        opts.headers = opts.headers?.set('Content-Type', CONTENT_TYPE_JSON);
      }
    }
